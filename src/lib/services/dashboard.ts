...
/**
 * Dashboard: encapsula las 8 queries que antes vivían inline en
 * `src/routes/(app)/+page.server.ts`.
 *
 * Devuelve el mismo shape que la ruta retornaba:
 * supplierCount, productCount, receivingCountThisMonth, shippingCountThisMonth,
 * lowStockItems, todayReceiving, todayShipping.
 */
export async function loadDashboard(ctx: ServiceCtx) {
	const db = ctx.db;
	...
	const [[sc], [pc], [rc], [shc], lowStockRows, todayReceiving, todayShipping] = await Promise.all([
	...
	const lowStockEnabled = (await getSettings(ctx)).low_stock_alert_enabled;
