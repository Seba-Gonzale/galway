...
export async function createCategory(ctx: ServiceCtx, data: { name: string; description: string | null }) {
	...
	try {
		...
		await auditLog(ctx, 'create', 'category', { target_label: name });
		return { success: true };
	} catch (err) {
		return handleDbError(err, 'category', 'create', 'That category name is already in use');
	}
}
...
export async function updateCategory(ctx: ServiceCtx, data: { id: string; name: string; description: string | null }) {
	...
	try {
		...
		await auditLog(ctx, 'update', 'category', { target_id: data.id, target_label: name });
		return { success: true };
	} catch (err) {
		return handleDbError(err, 'category', 'update', 'That category name is already in use');
	}
}
...
export async function deleteCategory(ctx: ServiceCtx, id: string) {
	...
	try {
		...
		return handleDbError(err, 'category', 'delete');
	}
}
